...
export default function ChatPreview({ chat, receiver }) {
  ...
  const [unreadCount, setUnreadCount] = useState(
    utilService.getUnreadMsgCount(chat.msgs, user._id)
  )
  ...
  const timeLastMsgSent =
    msgsLength > 0 ? formatTime(msgs[msgsLength - 1].sentAt) : null

  return (
    <div
      onClick={onSetCurrentChat}
      className={`chat-preview-container flex ${activeClass}`}
    >
      <UserProfileImage user={receiver} />
      <div className='chat-preview-details-container'>
        <h1>{receiver.username}</h1>
        <p>{sub}</p>
      </div>
      <div className='time-unread-container'>
        {timeLastMsgSent && (
          <span className='last-msg-sent-time'>{timeLastMsgSent}</span>
        )}
        {unreadCount > 0 && (
